		...
		;(async () => {
			try {
				if (formSubmitted && Email !== "" && Password !== "") {
					...
					if (
						...
						if (token) {
							...
							let testName =
								age >= 0 && age <= 15 ? "Screening" : "PSC"
							dispatch(setPscOrScreen(testName))
							localStorage.setItem(
								"user_complete_information",
								JSON.stringify(res.data.data)
							)
							// IF PATIENT ALREADY GIVEN THE TEST THEN IT WILL REDIRECT TO PATIENT DASHBOARD
					...
					// THIS IS FOR DOCTOR
						...
						if (token) {
							...
							if (res?.data?.data) {
								...
								const getDoctorCompleteProfileRes =
									await axios.get(
								...
								if (
									...
									let {
										...
										getDoctorCompleteProfileRes?.data
											?.data[0]
									let myObj = {
										...res?.data?.data,
										doctor_details,
				...
				if (error.response.data.status === 307) {
					...
					toast.error("Redirecting To OTP verification")
		...
		<Container className="login__section">
			...
			<Form onSubmit={formik.handleSubmit} className="login__form mt-5">
				...
				<Row className="text-center">
					<span style={{ fontSize: "14px" }}>
						{/* <Link to="/register" className="account__link"> */}
						<Link to="/forgot-password" className="account__link">
							Forget Password?
						</Link>
					</span>
				</Row>
